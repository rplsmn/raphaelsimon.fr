---
title: Les skills comme des packages
...
- la spécification des skills est l'une des meilleures idées que j'ai vues et utilisées jusqu'ici
- ça fait exactement ce que les packages font avec mes langages préférés : factoriser des trucs chouettes en briques réutilisables qu'on peut appeler en quelques mots
- c'est encore immature et ça devrait clairement évoluer pour se comporter comme des packages

Je bidouille autour de cette approximation skills-comme-packages et les résultats actuels me plaisent assez pour en parler, ne serait-ce que pour me relire dans quelques mois et constater à quel point cette prise de position vieillit mal. Ma vision actuelle, c'est un agent kit manager (akm).

## Les skills, c'est vraiment une super idée
...
Tout le monde bidouillait du prompt engineering et réutilisait des bouts qui marchaient dans des prompts de plus en plus longs, à coups de copier-coller et de renvois vers des documents structurants. Les skills répondent à tout : le dévoilement progressif permet de garder du contexte pour l'essentiel avant d'atterrir dans la [zone débile](https://www.youtube.com/watch?v=rmvDxxNubIg), l'auto-activation fait qu'on ne gaspille ni tours ni tokens à invoquer explicitement la skill, et l'arborescence de dossiers permet d'aller encore plus loin dans le dévoilement progressif, avec des fichiers lus uniquement dans certaines situations, des scripts utilisés par la skill...
...
À ce jour, j'ai probablement essayé une centaine de skills. Je dirais environ 80 trouvées en ligne et 10 à 20 construites sur mesure pour un repo ou une situation. Elles orientent l'agent vers des méthodologies de travail comme le lean, le TDD, comment écrire des ADRs, comment faire de la revue de code, comment utiliser un package R ou JS spécifique... Ça marche !
...
Même avec le dévoilement progressif, à un ratio ridiculement impossible d'1 token/skill, ça fait toujours plus de tokens que la fenêtre de contexte d'Opus, le double de sa "smart window" !
...
[^1]: Si vous ne connaissez pas celui-là, il vous manque quelque chose de crucial. La skill de brainstorming est probablement la skill qui m'a apporté le plus de valeur jusqu'ici.

En plus, toutes ces skills devaient être copiées-collées aux bons endroits : .claude, .github, ah non c'est .agents, attends je la veux en global, ah non ça crée de la confusion dans ce repo, ATTENDS cette machine n'a aucune de mes skills globales !!
...
Les skills sont des packages pour LLM, et je pense qu'il faut appliquer les mêmes standards. Tous les langages open source populaires ont des packages et un gestionnaire de packages. Des bouts de code que quelqu'un a empaquetés pour que le programme fasse un truc cool, et qu'on invoque en quelques mots. Factoriser le comportement. On décrit les dépendances dans des fichiers manifeste, on s'appuie sur le versionnage, et un ou plusieurs registres pour héberger les dernières versions. C'est le cœur de l'open source : centraliser les efforts, imposer des standards de documentation, et en retour gagner en reproductibilité, en distribution, en commandes d'install / update. Python a PyPi, Node a npm, R a CRAN, Rust a Cargo... Évidemment l'analogie s'effondre parce que les skills ne sont pas du vrai code, mais c'est largement suffisant pour justifier un traitement similaire.
...
- Un système de gestion de la portée des skills
...
C'est assez simple : les skills sont rangées dans .local/share/llm-skills sur chaque machine, copie unique, invisibles pour les outils CLI.

Je génère ensuite un fichier library.json à partir de l'ensemble des dossiers, et je peux ajouter des variables à chaque skill : est-elle destinée au dossier global (core: true) ? Quels sont ses tags (pour la recherche) ?
...
Ce que ça fait : charger à chaud une skill dans le répertoire temporaire, *sans* l'ajouter au manifeste.

Qu'est-ce que j'essaie d'accomplir avec ça ?

D'abord, je veux que des agents dans des sessions séparées puissent utiliser des skills nécessaires uniquement pour leur session, sans modifier le manifeste du repo. Une tâche spécifique a besoin d'une skill très niche ?
Mon graal, c'est d'arriver à des skills "juste à temps" (JIT), peut-être même en *construire* une à la volée avec une équipe d'agents imbriqués qui itèrent dessus.
...
- Le versionnage : si les skills sont des packages, j'aimerais définir une sémantique de version pour les skills. Ça permettrait de déclarer / récupérer des dépendances exactes pour chaque projet depuis le manifeste.
